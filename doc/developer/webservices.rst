...
* ``sets``: with kind of data we want to get, can be ``all``, ``themes``, ``group``
  or ``background_layers``, default is ``all``.
* ``background_layers_group``: get only one group, used to get the base layers.
